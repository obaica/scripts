...
def formatter(infile='POSCAR',outfile='POSCAR_new'):
    ...
    #reformatted array
    new_positions = np.concatenate((positions[0:num_prev_atoms],positions[num_prev_atoms+num_vac:]),axis=0)
    ...
    for i_pos in range(len(new_positions)):
        p_file.write('%f %f %f \n' %(new_positions[i_pos,0],new_positions[i_pos,1],new_positions[i_pos,2]))
